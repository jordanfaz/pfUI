...
`ClassicAPI.dll`, which is listed in `dlls.txt` under VanillaFixes — and verified at runtime 2026-08-21 with the on-disk folder deleted: clean load with no errors, and `CreateColor`, `ColorMixin`, `Mixin`, `C_CreatureInfo`, `GetCoinTextureString` and `GetPlayerInfoByGUID` all resolving. The sync's own additions check out on the same pass: the `C_CreatureInfo` race loop terminates and populates (10 entries, `Human` → Human/Alliance), and `GetCoinTextureString` money renders as real coin textures everywhere, chat frame included — the raw `|T...|t` escape only shows up when copying the line out, which is a copy artefact and not a rendering failure. The count is
...
| `modules/nameplates.lua` | The level text was brightened by `+0.3` on all three channels before display — once in the `levelFromDB` path, again in the OnUpdate colour sync. A flat offset desaturates toward white and costs the high tiers most, because their green/blue channels start near zero: `verydifficult` 1.00/0.50/0.25 → 1.00/0.80/0.55 (reads **yellow**) and `impossible` 1.00/0.10/0.10 → 1.00/0.40/0.40 (reads **orange**), leaving orange and yellow 0.2 apart on a single channel. So a mob 3–4 levels above you showed yellow instead of orange, and a skull-range mob showed orange. Second defect in the same pair: the `levelFromDB` write never updated `plate.cache.levelcolor`, and `nameplate.cache` is built once at plate **creation** and survives pool reuse (`OnShow` re-runs the data pass but never clears it) — so after a `??` plate the cached channel sum was stale, and a frame recycled onto a unit whose Blizzard colour matched it kept the *previous* unit's colour. Both offsets dropped, ownership made explicit through `cache.levelfromdb`. Deleting the sync block outright — the obvious fix — is wrong: it is the only thing that colours the level on non-`??` plates, since the `GetDifficultyColor` path is gated behind `levelFromDB`, and `plate.level` is created with no colour at all. Reported by Iden via Discord 2026-08-19, who also identified the `+0.3` as the cause. Verified in game 2026-08-21 at level 3: Prairie Wolf 6 (+3) and Adult Plainstrider 7 (+4) both report `1.00 0.50 0.25` — true orange, where the old code returned `1.00 0.80 0.55`. (`eaa9beac`) |
| `modules/nameplates.lua` | Three defects in the friendly-name colour path. **One:** `cache.namecolor` was written by two writers holding unrelated quantities — the computed health-bar colour in OnDataChanged (class-coloured friendly names, `friendclassnamec`) and Blizzard's own name FontString in the OnUpdate sync — so each could suppress the other through the shared key and the option silently lost: names came out white with `namefightcolor` off, or the plain reaction colour with it on. **Two:** the sync writer sets `update = true` whenever it writes, so the pair flipping the key forced a full data pass every throttled tick, defeating the half-second tick gate; and since `nameplate.cache` survives pool reuse, a recycled plate could inherit the previous unit's name colour. Ownership is now explicit through `cache.ownname`, each writer keeps its own key, and both are invalidated when ownership flips. (`0a4b3a7d`) **Three:** the write sourced `r, g, b` from the health bar rather than `PFUI_CLASS_COLORS[class]`, which the condition tested for but never read — so class colours on names only appeared when `friendclassc` (a *bar* option) happened to be on too, and the name also inherited the tapped-grey and `barcombatstate` overrides meant for the bar. It now reads the class table directly; the discarded `and PFUI_CLASS_COLORS[class]` term was dead weight, since that table's `__index` returns a grey ColorMixin for any missing key and so was always truthy. A friendly player of an unrecognised class now takes that grey fallback rather than the bar colour, which is what the option means. (`f880dc35`) Same shared-ancestor code as the level colour above — byte-identical in the pre-rebase July fork — so all three are live upstream. Invisible on a default install: `friendclassnamec` ships off. Verified in game 2026-08-21 in a crowded city with `friendclassnamec` on and `friendclassc` deliberately **off**, the case that could not work before: 37 plates across seven classes, every friendly player name reporting its exact class colour while the bars sat on the generic `FRIENDLY_PLAYER` blue, and NPCs and pets keeping Blizzard's passthrough white. |

---

### Defaults kept against upstream
