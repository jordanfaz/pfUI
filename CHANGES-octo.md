...
| `api/api.lua` + `init/modules.xml` | ~~`GetPerfectPixel` measured against the `uiScale` CVar, which caps at 1.0 while the Huge/Large presets push `UIParent` past it.~~ **Wrong — reverted 2026-08-10.** The cap is load-bearing, not a bug: at the true scale (1.42 on Huge) `768/screenheight/scale` falls below half a pixel, `edgeSize` rounds to zero and borders vanish entirely. Merged upstream, then reverted by brues in `9cabdf29`. The `gxResolution` nil guard in the same function was sound and stays. |
